...
Class ModeloBlog{
	...
	static public function mdlBuscador($tabla1, $tabla2, $desde, $cantidad, $busqueda){
		...
		fecha_articulo FROM $tabla1 INNER JOIN $tabla2 ON $tabla1.id_categoria = $tabla2.id_cat WHERE titulo_articulo 
		like '%$busqueda%' OR descripcion_articulo like '%$busqueda%' OR contenido_articulo 
		like '%$busqueda%' OR ruta_articulo like '%$busqueda%' ORDER BY $tabla2.id_articulo DESC LIMIT $desde, $cantidad");
